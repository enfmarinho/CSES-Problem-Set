...
int main() {
  ...
  for (int index{0}; index < n; ++index) {
    cin >> tabel[index].first;
    tabel[index].second = index + 1;
  }
  sort(range(tabel));

  auto first = tabel.begin();
  auto last = --tabel.end();
  while (first != last) {
    int sum = first->first + last->first;
    if (sum > x) {
      --last;
    } else if (sum < x) {
      ++first;
    } else {
      cout << first->second << ' ' << last->second;
      return 0;
    }
  }
  cout << "IMPOSSIBLE";
