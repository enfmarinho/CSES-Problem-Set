...
int main() {
  ...
  for (int index{0}; index < n; ++index) {
    cin >> time[index].first >> time[index].second;
  }
  sort(range(time));
  ...
  set<int> exit;
  for (int index{0}; index < n; ++index) {
    exit.erase(exit.begin(), exit.lower_bound(time[index].first));
    exit.insert(time[index].second);
    ans = max(ans, exit.size());
  }
  cout << ans;
  return 0;
}
