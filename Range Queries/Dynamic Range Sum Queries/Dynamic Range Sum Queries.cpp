...
ll build(ll index, ll start, ll end) {
  ...
  return segtree[index] =
             build(index * 2, start, mid) + build(index * 2 + 1, mid + 1, end);
}
...
ll query(ll interval_begin, ll interval_end, ll index, ll start, ll end) {
  ...
  ll mid = (start + end) / 2;
  return query(interval_begin, interval_end, 2 * index, start, mid) +
...
ll update(ll position, ll value, ll index, ll start, ll end) {
  ...
  int mid = (start + end) / 2;
  return segtree[index] = update(position, value, 2 * index, start, mid) +
...
int main() {
  ...
  while (q--) {
    ...
    if (type == 1) {
      ...
      --second;
      cout << query(first, second, 1, 0, n - 1) << '\n';
    }
  }
  return 0;
}
